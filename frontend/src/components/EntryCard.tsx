    ...
    <div className="card relative">
      ...
      {modalClicked && (
        <>
          ...
          <div
            className={
              modalClicked
                ? "fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white p-6 rounded-md w-fit shadow-md border-2 border-solid border-red-600 z-50"
                : "hidden"
            }
          >
            ...
            <div className="flex flex-end gap-3 justify-center mt-10">
              <button
                onClick={toggleModal}
                className="text-[14px] bg-neutral-200 p-2 pl-3 pr-3 rounded-md cursor-pointer hover:bg-neutral-300 transition"
              >
                Cancel
              </button>
              <button
                aria-label="yes, i'm sure"
                onClick={handleDelete}
                className="text-[14px] bg-red-600 text-red-50 p-2 pl-3 pr-3 rounded-md cursor-pointer hover:bg-red-700 transition"
              >{`Yes, I'm sure`}</button>
            </div>
          </div>
        </>
      ...
      <p
        className={
          isEditing
            ? "hidden"
            : "text-dark-brown/80 leading-relaxed whitespace-pre-wrap"
        }
      >
        {entry.content}
      </p>
      <div className="flex items-center justify-between text-xs text-warm-gray mt-4">
        ...
        <div className="flex gap-2">
          <button aria-label="edit entry" onClick={() => setIsEditing(true)}>
            <svg
              className={
                isEditing
                  ? "hidden"
                  : "p-1 rounded-md cursor-pointer hover:bg-blue-100 hover:fill-blue-800 transition"
              }
          ...
          <button aria-label="delete entry" onClick={toggleModal}>
            <svg
              className={
                isEditing
                  ? "hidden"
                  : "p-1 rounded-md cursor-pointer hover:bg-red-100 hover:fill-red-800 transition"
              }
              xmlns="http://www.w3.org/2000/svg"
              height="28px"
              viewBox="0 -960 960 960"
